...
import proposalclasses.ListDecision;
...
public class FXMLPrerequisiteSetterController implements Initializable {
    ...
    @FXML
    private ListView<ListDecision> listMust;
    @FXML
    private ListView<ListDecision> listNot;
    ...
    public void init(ArrayList<Integer> must, ArrayList<Integer> not, int[] min_resources, boolean[] applicant_roles, boolean[] recipient_roles, ArrayList<ArrayList<String>> s, boolean i, TreeSet<Decision> d) {
        ...
        ArrayList<ListDecision> must_array_list = new ArrayList<ListDecision>();
        for (Decision each : d) {
            must_array_list.add(new ListDecision(each.title, false, each.id));
        }
        ArrayList<ListDecision> not_array_list = new ArrayList<ListDecision>();
        for (Decision each : d) {
            not_array_list.add(new ListDecision(each.title, false, each.id));
        }

        ObservableList<ListDecision> must_list = FXCollections.observableArrayList(must_array_list);
        ObservableList<ListDecision> not_list = FXCollections.observableArrayList(not_array_list);

        listMust.setItems(must_list);
        listNot.setItems(not_list);

        listMust.setCellFactory(CheckBoxListCell.forListView((ListDecision item) -> item.on()));
        listNot.setCellFactory(CheckBoxListCell.forListView((ListDecision item) -> item.on()));

        //CHECK MUST DECISIONS
        for (Integer each_id : must) {
            for (ListDecision each_decision : listMust.getItems()) {
        ...
        for (Integer each_id : not) {
            for (ListDecision each_decision : listNot.getItems()) {
                if (each_decision.id() == each_id) {
                    each_decision.check(true);
                    break;
                }
    ...
    private void handleBtnOk(ActionEvent event) {
        ...
        for (ListDecision each_must : listMust.getItems()) {
            for (ListDecision each_not : listNot.getItems()) {
        ...
        //ADD MUST
        for (ListDecision each : listMust.getItems()) {
            if (each.on().get()) {
        ...
        //ADD NOT
        for (ListDecision each : listNot.getItems()) {
            if (each.on().get()) {
